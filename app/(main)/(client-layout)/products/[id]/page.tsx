...
export const getData = async (params: { id: string }) => {
  ...
  const productCategoryID = (product as any).category.pop()
  ...
  return {
    ...
    similarProducts: {
      ...
      products: JSON.parse(JSON.stringify(similarProducts)),
    },
  }
}
...
const SingleProduct = async ({ params }: Props) => {
  const { product, similarProducts } = await getData(params)
  ...
  return (
    <main className="xl:mt-28 container mx-auto py-4 space-y-4">
      ...
      <SimilarProductsSlider similarProducts={(similarProducts as any)} />
