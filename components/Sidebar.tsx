...
export default function Sidebar() {
  ...
  const { categoriesList, isLoading } = useGetCategoriesQuery(undefined, {
    selectFromResult: ({ data, isLoading }) => ({
      //@ts-ignore TODO: figure out where the categoriesList is coming from
  ...
  return (
    <>
      ...
      <div
        className={`w-full h-screen fixed duration-200 z-10 top-0 lg:hidden ${isSidebar ? 'right-0' : '-right-full'
          } `}
      >
        <div
          className={`${isSidebar ? 'opacity-100 visible duration-300 delay-200' : 'opacity-0 invisible '
        ...
        <div className="overflow-y-auto absolute py-4 top-0 right-0 z-20 w-3/4 h-screen max-w-sm space-y-4 bg-white">
          ...
          <h5 className="p-3 border-t-2 border-gray-200">Product Categories</h5>
          {isLoading ? (
            <SidebarSkeleton />
          ) : categoriesList ? (
            <div>
              {categoriesList.children &&
                categoriesList.children.map((category: Category) => (
                    ...
                    {({ open }) => (
                      <>
                        <Disclosure.Button className="flex items-center justify-between px-4 py-2 w-full !mt-0">
                          ...
                          <Icons.ArrowDown
                            className={` ${open ? 'rotate-180 transform text-red-400 ' : 'text-gray-700'
                        ...
                        <Disclosure.Panel className=" text-sm bg-gray-100 text-gray-500 !mt-0">
                          ...
                          {category?.children &&
                            category.children.map((category: Category) => (
                              <Disclosure key={category._id}>
                                {({ open }) => (
                                  <>
                                    <Disclosure.Button className="flex items-center justify-between px-4 py-2 w-full !mt-0 pl-7">
                                      <span
                                        className={`font-medium text-md ${open ? 'text-red-400' : 'text-gray-600'
                                          }`}
                                      >
                                        {category.name}
                                      </span>
