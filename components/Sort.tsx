...
interface SortProps {
  handleChangeRoute: (data: { sort?: string }) => void
}

const Sort = ({ handleChangeRoute }: SortProps) => {
  //? Assets
  ...
  //? Handlers
  const handleSortChange = (item: { name: string; value: number }) => {
    setSort(sorts[item.value - 1])
    //@ts-ignore TODO: the item value is a number above but a string below as thhe handleChangeRoute expects a string
