import { RootState, setTempColor } from '@/store'
...
const SelectColor = ({ colors }: SelectColorProps) => {

  //? Assets
  const dispatch = useAppDispatch()

  //? Store
  const { tempColor } = useAppSelector((state: RootState) => state.cart)

  //? Render(s)
  return (
    <section className="">
      <div className="flex justify-between p-4">
        <span className="text-sm text-gray-700">Color: {tempColor?.name}</span>
        <span className="text-sm">{colors.length} colors</span>
      </div>
